...
- `summary-json` now has an explicit 256 KiB UTF-8 cap. Results that exceed it no longer fail an otherwise successful B2 operation or emit a partial array under `summary-json`; instead the Action sets `summary-json-truncated=true`, writes a small truncation notice object to `summary-json`, and emits a bounded `summary-json-preview` for diagnostics. Migration note: consumers that parse `summary-json` as an array must branch on the always-emitted `summary-json-truncated` output first, and use `file-count` plus verb-specific count outputs for authoritative totals when it is `true`. ([#41](https://github.com/backblaze-labs/b2-action/issues/41))
- `presign` no longer writes live presigned download URLs into `summary-json` or `summary-json-preview`; the dedicated `presigned-url` output remains the bearer-URL channel. ([#41](https://github.com/backblaze-labs/b2-action/issues/41))
